...
struct segtree{
    ...
    int minimin(int l , int r , int x , int lx , int rx){
        if(rx <= l || lx >= r) return INT_MAX;
        if(lx>=l && rx <= r) return mini[x];
        int m = (lx+rx)/2;
        int s1 = minimin(l,r,2*x+1,lx,m);
        int s2 = minimin(l,r,2*x+2,m,rx);
        return min(s1,s2);
    }
    int minimin(int l, int r) {return minimin(l,r,0,0,size);}
};
